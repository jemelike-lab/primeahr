...
function generatePreviewHTML(offer: any) {
  const name = offer.candidate?.first_name || "[Candidate]";
  const pos = offer.position_title || "[Position]";
  const dept = offer.department ? " in the <strong>" + offer.department + "</strong> department" : "";
  const sal = offer.salary ? "<p>Your compensation will be <strong>$" + Number(offer.salary).toLocaleString() + "</strong> per year.</p>" : "";
  const sd = offer.start_date ? "<p>Your anticipated start date is <strong>" + new Date(offer.start_date).toLocaleDateString("en-US", {month:"long",day:"numeric",year:"numeric"}) + "</strong>.</p>" : "";
  const ben = offer.benefits_summary ? "<p><strong>Benefits:</strong> " + offer.benefits_summary + "</p>" : "";
  const et = (offer.employment_type || "full_time").replace("_", " ");
  return "<div style=\"font-family:system-ui;line-height:1.7\"><div style=\"text-align:center;margin-bottom:2rem\"><h2 style=\"color:#064e3b;margin:0\">Beatrice Loving Heart</h2><p style=\"color:#6b7280;font-size:0.85rem\">Home Health & DDA Services</p></div><p>Dear " + name + ",</p><p>We are pleased to offer you the position of <strong>" + pos + "</strong>" + dept + " at Beatrice Loving Heart.</p>" + sal + sd + ben + "<p>This position is classified as <strong>" + et + "</strong>.</p><p>Please confirm your acceptance by signing below. We look forward to having you join our team.</p><p style=\"margin-top:2rem\">Sincerely,<br/><strong>Beatrice Loving Heart HR Team</strong></p></div>";
}